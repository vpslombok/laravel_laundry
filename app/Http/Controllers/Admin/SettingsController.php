...
class SettingsController extends Controller
{
  ...
  {
    $set_target = LaundrySetting::where('user_id', $id)->first();
    if (!$set_target) {
      $set_target = new LaundrySetting();
      $set_target->user_id = $id; // Tambahkan ini untuk mengaitkan dengan user_id
    }
  ...
  {
    $notif = notifications_setting::where('user_id', $id)->first();
    if (!$notif) {
      $notif = new notifications_setting();
      $notif->user_id = $id;
    }
    $notif->telegram_order_masuk = $request->telegram_order_masuk ?? 0;
    $notif->telegram_order_selesai = $request->telegram_order_selesai ?? 0;
