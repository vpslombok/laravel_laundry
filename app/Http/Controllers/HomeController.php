    ...
    public function index()
    {
        if (Auth::check()) {
            if (Auth::user()->auth === "Admin") {
                $masuk = transaksi::whereIN('status_order', ['Process'])->count();
                $selesai = transaksi::where('status_order', 'Done')->count();
                $diambil = transaksi::where('status_order', 'DiTerima')->count();
                ...
                $bln = DB::table('transaksis')
                    ->selectRaw('MONTH(created_at) as bulan, COUNT(id) as jml')
                ...
                $masuk = transaksi::whereIN('status_order', ['Process'])->where('user_id', auth::user()->id)->whereMonth('created_at', date('m'))->count();
                $selesai = transaksi::where('status_order', 'Done')->where('user_id', auth::user()->id)->whereMonth('created_at', date('m'))->count();
                $diambil = transaksi::where('status_order', 'DiTerima')->where('user_id', auth::user()->id)->whereMonth('created_at', date('m'))->count();
                ...
                $lastUpdated = transaksi::where('user_id', Auth::user()->id)
                    ->latest('updated_at')
                    ->value('updated_at');
